...
export default new CustomCommand({
  ...
  execute: async (message) => {
    ...
    if (parsed.animeID || parsed.animeId) {
      try {
        const animeOnAnima = await fetch(
          `https://api.anima.moe/anime/${parsed.animeID || parsed.animeId}`,
        )
        const animeOnAnimaJSON = await animeOnAnima.json()

        embed.setThumbnail(animeOnAnimaJSON.data.cover)
        const animeTitle = animeOnAnimaJSON.data.AnimeMetadata.find((
          a: { locale_key: string },
        ) => a.locale_key === message.locale)?.title ||
          'Anime not found / not translated'
